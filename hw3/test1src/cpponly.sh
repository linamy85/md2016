...
if [ "$1" == "-h" ]; then
  echo "Usage: $0 [directory] [valid? (y/n)] [iter] [session id]"
  exit 0
fi
...
if [ ! -f $MATRIX_DIR/source.matrix.1 ]; then
  ...
  $LIBFM/bin/libFM -task r -train $MATRIX_DIR/source.libfm.0 \
    -test $MATRIX_DIR/full.libfm -dim '1,1,30' -out $MATRIX_DIR/source.matrix.1 \
    -iter $3 &
fi
...
if [ "$2" == "y" ] || [ ! -f $MATRIX_DIR/train.matrix.$session ]; then
  ...
  $LIBFM/bin/libFM -task r -train $MATRIX_DIR/train.libfm.$session \
    -test $MATRIX_DIR/full.libfm -dim '1,1,30' -out $MATRIX_DIR/train.matrix.$session \
    -iter $3 &
  sleep 3
fi
...
g++ -Wall -c -O3 -std=c++11 $SRC_DIR/Kuhn_Munkres.cpp
g++ -Wall -c -O3 -std=c++11 -pthread $SRC_DIR/step23.cpp

g++ -Wall -pthread step23.o Kuhn_Munkres.o -o step23

./step23 $MATRIX_DIR/source.matrix.1 $1/source.txt \
  $MATRIX_DIR/train.matrix.$session $1/train.txt $MATRIX_DIR/mytrain.libfm

rm step23 step23.o hungarian.o
...
python $SRC_DIR/tolibfm.py $1/test.txt $MATRIX_DIR/test.libfm n

$LIBFM/bin/libFM -task r -train $MATRIX_DIR/mytrain.libfm \
  -test $MATRIX_DIR/test.libfm -dim '1,1,30' -out $1/test.out \
  -iter $3
