...
class Model:
    ...
    def train(self):
        ...
        partition = 5
        threads = []
        for list in np.split(np.array(range(N_USER)), partition):
            thread = Thread(target = self.threadFunc, args = (trans, list))
    ...
    def threadFunc (self, trans, list):
        period = len(list) // 10
        count = 0
        for user2 in list:
            # Prints status.
            count += 1
            if count % period == 0:
                print list[0], "thread] :", (count / period * 10), "%"

            # Graps the non-zero items
            nonzero = []
            for i in range(N_ITEM):
                if self.r2[user2][i] > 0.0:
                    nonzero.append((i, self.r2[user2][i]))
            ...
            for user1 in range(N_USER):
                # Counts dot.
                dot = 0.0
                for item, w in nonzero:
                    dot += w * trans[user1][item]
